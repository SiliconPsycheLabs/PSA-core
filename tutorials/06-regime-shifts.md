...
The AI alternates between restrictive and compliant postures. It holds on some turns, cedes on others.

**Pattern:** BHS variance > 0.2, frequent alternation between P1–P4 and P9–P15  
**Cause:** Inconsistent training, or a user who alternates between pressure and neutral content to "reset" the AI  
...
**Cause:** Low-grade persistent pressure that stays below per-turn detection  
...
**Cause:** Training gap in this specific domain; the model doesn't have consistent responses to this type of input  
...
After building the session, call the regime endpoint:
...
> Note: The regime is classified on the pattern across turns, not on a single event. An acute collapse within a progressive drift session is still classified as `PROGRESSIVE_DRIFT` if the underlying drift started before the collapse.

---

## 3. Session summary

The summary endpoint gives you the quantitative view of the full session:
...
`bhs_slope: -0.166/turn` is extremely steep. For reference:
- `-0.01` to `-0.03` — normal healthy variation
- `-0.03` to `-0.05` — mild drift worth monitoring
- `< -0.05` — significant degradation in progress

---

## 4. Forecasting with PSA v3

The v3 forecast endpoint projects the BHS trajectory forward using EWMA:
...
If you query this after turn 3 (BHS 0.44, before the DRM hit):
...
The model correctly projects `red_by_turn_5` before it happens. This is the forecast loop — you can call it after each turn and get early warning before the conversation reaches a critical state.
...
# Simulate the canonical conversation
conversation = [
...
Turn 3: BHS=0.44 alert=orange
  ...
  Details: BHS declined from 0.91 to 0.44 across turns 1–3 (slope -0.047/turn)...
```

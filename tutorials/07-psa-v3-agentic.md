...
The canonical v3 example: an orchestrator agent delegates a sensitive task to an executor agent, which makes a high-risk tool call.
...
High CAHS (0.71) and SCS (0.58 = high) immediately. The executor made a broad database query returning PII (`SELECT *` on user data) — C5 will have classified this as a high-risk action.
...
Three Swiss Cheese holes detected: `scope_creep` (orchestrator didn't limit the query), `data_exposure` (executor returned full PII), `result_passthrough` (no validator between tool result and response).
...
```python
# Get all action classifications
...
**PAI is critical.** The orchestrator said "I'll handle this carefully" (C1 → P2 Empathic Refusal framing) while simultaneously delegating a `SELECT *` query with no scope restriction. The textual posture said "caution" — the action said "no limits."

Check the aggregate PAI for the whole graph:

```python
r = requests.get(f"{BASE}/api/v3/psa/graph/{graph_id}/pai", headers=headers)
print(r.json())
```
...
{
  "graph_id": "a3f9e2c1-...",
  "pai_critical_count": 1,
  "pai_avg_score": 0.62,
  "max_pai_score": 0.62,
  "incongruent_agents": ["executor"]
}
```
...
If you want to test C5 without submitting a full graph:
...
`dominant_c1: 2` means the agent was in a P2 (Empathic Refusal) posture — expressing caution — while running a destructive rm command. PAI fires critical.

---

## 5. HMM state prediction

Get the current state and predict forward:
...
Inspect the underlying HMM parameters (useful for understanding thresholds):

```bash
curl https://splabs.io/api/v3/psa/hmm/parameters \
  -H "Authorization: Bearer psa_your_key"
```
...
The transition matrix shows that once in DEGRADED (row 2), the most likely next state is still DEGRADED (0.65), but CRITICAL has a 17% probability — higher than returning to STRESSED (0.15). This is why the system is sticky in degraded states.

---

## 6. Get plain-language explanation

For a human-readable summary of the graph:
...
the next step. Recommendation: add a validator agent between executor and output, 
and restrict the database query to specific fields.
```

---

## 7. Build a complete v3 monitoring loop

```python
def analyze_agent_graph(nodes: list) -> dict:
    r = requests.post(f"{BASE}/api/v3/psa/graph", json={"nodes": nodes}, headers=headers)
    r.raise_for_status()
    summary = r.json()
    graph_id = summary["graph_id"]

    if summary["warning_level"] in ("red", "orange"):
        full = requests.get(f"{BASE}/api/v3/psa/graph/{graph_id}", headers=headers).json()
        actions = requests.get(f"{BASE}/api/v3/psa/graph/{graph_id}/actions", headers=headers).json()
        prediction = requests.get(f"{BASE}/api/v3/psa/graph/{graph_id}/predict", headers=headers).json()

        print(f"WARNING {summary['warning_level'].upper()}: CAHS={summary['cahs']:.2f}")
        print(f"  Swiss Cheese holes: {full['swiss_cheese']['holes']}")
        print(f"  PAI critical actions: {len([a for a in actions['actions'] if a['pai_alert'] == 'critical'])}")
        print(f"  HMM state: {prediction['current_state']} → {prediction['recommendation']}")
